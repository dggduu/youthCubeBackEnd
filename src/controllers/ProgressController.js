...
export const progressController = {
   ...
   * @access Private（仅本人）
   */
  updateProgressComment: async (req, res) => {
    try {
      const { id } = req.params;
      const { content } = req.body;
      const user_id = req.user.userId;

      const comment = await getCommentOr404(id, res);
      if (!comment) return;

      if (!checkPermission(res, comment.user_id, user_id)) return;

      await comment.update({ content });

      const updatedComment = await ProgressComment.findByPk(id, {
        include: [{ model: User, as: 'author', attributes: ['id', 'name', 'avatar_key'] }]
      });

      res.status(200).json({ message: '评论已更新', comment: updatedComment });
    } catch (error) {
   ...
   * @access Private（仅本人）
   */
  deleteProgressComment: async (req, res) => {
    try {
      const { id } = req.params;
      const user_id = req.user.userId;
